...
def test_drive_blink_fault_vehicle(can_pair):
    ...
    for msg in drain(rx, count=8):
        ...
        elif arb == CAN["CAN_PDC_MPH"] and len(data) >= 4:
            world["mph"] = unpack_float(data)
        elif arb == CAN["CAN_BPS_ELECTRICAL"] and len(data) >= 6:
    ...
    if world["fault"]:
        ...
        world["bps_light"] = bool(status.data[0] & CAN["CAN_PT_FAULT_MASK"])
